...
EXP_SCRIPT=src/experiments_sharing_subset.py
...
mpirun -hostfile $COBALT_NODEFILE -n $PROCS -npernode $PPN singularity exec --nv -B $MPI_BASE --pwd $EXP_DIRECTORY $CONTAINER python $EXP_SCRIPT --supervision --dataset=cifar --model=resnet --local_ep=1 --num_users=5 --frac=1